...
drop table if exists dws.tmp_report_revenue_part_two_add;
create table if not exists dws.tmp_report_revenue_part_two_add (
...
tmp_t as(
  ...
  where a.product_type in ('保险类产品', '现金类产品')
...
tmp_csc1 as (
    ...
    select
    apply_no, to_date(trans_day) d_trans_day, sum(trans_money) trans_money
...
tmp_charge_amount as (
     ...
     from(
          ...
          where b.d_trans_day = date_sub(from_unixtime(unix_timestamp(),'yyyy-MM-dd'), 1)
          GROUP BY
          a.company_name
          ,a.product_name
          ,a.product_type
...
tmp_lrfy as (
    ...
    group by a.company_name, a.product_name, a.product_type, a.product_name_c, a.is_overtime, a.create_date
)

insert overwrite table dws.tmp_report_revenue_part_two_add
select
a.create_date, a.company_name, a.product_name, a.product_type, a.product_name_c, a.is_overtime
    ...
    ,sum(case
         ...
         then
              case  when a.khhk_date = date_sub(from_unixtime(unix_timestamp(),'yyyy-MM-dd'), 1) and a.capital_use_time <= a.firstPartOfChargingTerm
                    then a.channelPricePerOrderFirstPart/100/a.firstPartOfChargingTerm*nvl(a.fk_amount, 0)
                         *(a.firstPartOfChargingTerm-a.capital_use_time+1)

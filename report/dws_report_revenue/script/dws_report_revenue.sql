...
create table if not exists dws.dwstmp_report_revenue (
...
with tmp_month as (
  select
  b.company_name, b.product_type, b.product_name_c, b.product_name, b.is_overtime 
  ...
  from dws.tmp_report_revenue_part_two b
  where b.create_date >= DATE_FORMAT(date_sub(from_unixtime(unix_timestamp(),'yyyy-MM-dd'), 1),"yyyy-MM-01") 
  group by b.company_name, b.product_type, b.product_name_c, b.product_name, b.is_overtime
),
tmp_csc1 as (
    select
...
tmp_lrfy as (
    select  irpo.company_name, irpo.product_name, irpo.product_type, irpo.product_name_c, irpo.is_overtime
    ...
    group by irpo.company_name, irpo.product_name, irpo.product_type, irpo.product_name_c, irpo.is_overtime
    union all
    -- 当日录入返佣金额-发票报销
    select irpo.company_name, irpo.product_name, irpo.product_type, irpo.product_name_c, irpo.is_overtime
    ,0 hsfy_amount
...
tmp_premium_amount as (
    SELECT
    a.company_name, a.product_name, a.product_type, a.product_name_c, a.is_overtime, a.apply_no
    ...
    GROUP BY a.company_name, a.product_name, a.product_type, a.product_name_c, a.is_overtime, a.apply_no
)

insert overwrite table dws.dwstmp_report_revenue
select
date_sub(from_unixtime(unix_timestamp(),'yyyy-MM-dd'), 1) create_date
,a.company_name ,a.product_type ,a.product_name ,a.product_name_c ,a.is_overtime
...
from (
    ...
    FROM
    dws.tmp_report_revenue_part_two AS a
    ...
    group by a.company_name ,a.product_name ,a.product_type ,a.product_name_c ,a.is_overtime
...
;
DROP TABLE IF EXISTS dws.dws_report_revenue;
ALTER TABLE dws.dwstmp_report_revenue RENAME TO dws.dws_report_revenue;

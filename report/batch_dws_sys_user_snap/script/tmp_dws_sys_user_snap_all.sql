-- 查询目前所有的数据（主要是因为系统的数据删除并不会反应在update_time中，所以需要以这种方式去除掉删除的数据。）
set hive.execution.engine=spark;
drop table if exists dws.tmp_dws_sys_user_snap_all;
create table if not exists dws.tmp_dws_sys_user_snap_all (
  user_id STRING
  ,   user_name STRING
  ,   user_status bigint
  ,   job_code STRING
...
insert overwrite table dws.tmp_dws_sys_user_snap_all
select
a.*

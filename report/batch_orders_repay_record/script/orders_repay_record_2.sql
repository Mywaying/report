...
select * from (
  select
    ...
    ,if (t666.returnconfirm_status_name='同意',t666.returnconfirm_time,NULL ) returnconfirm_time -- 资金归还确认时间
    ,tpg.create_time repaymentresult_time -- 资金归还核销时间
    ,case when (instr(bao.partner_insurance_name, '云南信托') > 0 or
     instr(bao.partner_insurance_name, '湖南信托-主动管理') > 0 or
     instr(bao.partner_insurance_name, '宝乾小贷') > 0 or
     instr(bao.partner_insurance_name, '光大信托') > 0
    )
 then
   cast (tpg.create_time as timestamp)
    ...
    left join ods_bpms_sys_dic tmp on tmp.key_ = bfs.product_term_and_charge_way WHERE tmp.type_id_ = '10000045260203') bfs on bao.apply_no = bfs.apply_no
    ...
    left join ods_bpms_biz_customer bc on bcr.customer_no = bc.cust_no    left join ods_bpms_biz_lm_fund_package blfp on bfs.fund_package_code = blfp.fund_package_code
    ...
    left join ods_bpms_c_fund_module_common cfm on bao.apply_no = cfm.apply_no    left join tmp_record_intervew tri on tri.apply_no = bao.apply_no
    where NOT EXISTS(select 1 from dwd.dwd_orders_repay_record where report_date='${var:logdate}' and category='returnconfirm') and bao.apply_status_name not in ('已终止', '拒绝') and (returnconfirm_status_name='同意'or tpg.create_time is not null)

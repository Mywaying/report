...
insert into table dws.stats_archive_order
select
  dwd.id dimension_date_id, -- 年月
  dp.id dimension_product_id,  -- 产品类型,交易类型
  dso.id dim_sys_org_id, -- 分公司
...
left join dim.dim_sys_org dso on dso.branch_id=tao.branch_id
left join dws.dimension_product dp on dp.product_code=tao.product_id
group by dwd.id,dp.id,dso.id;